...
If successful, Aegis Eye Agent can detect `DevicePolicyManager.isDeviceOwnerApp()` and use supported enterprise policies such as lock, app policy management, and system update policy where Android/OEM allows.
...
Modern Android does not allow normal apps or Device Admin apps to turn mobile data on/off. This requires OEM/system-level privileges. Aegis Eye Agent reports this limitation instead of pretending it succeeded.
...
## Aegis Eye Agent security hardening after Device Owner provisioning

When Aegis Eye Agent is successfully provisioned as Android Device Owner, the agent now enforces these supported enterprise policies on every heartbeat:
